...
##!   @file  - ge_eeprom.py
##
##!   @brief - This script allows users to read/write EEPROM page data on Icron's
##             Goldenears platforms using Icron's debugging protocals through
##             serial communication between a PC and a Goldenears product.
##
...
def print_usage():
    ...
    sys.stderr.write("Usage: \n" +
           "\tge_eeprom.py [serial_port_name] read > [output_file] \n" + \
           "or \n" + \
           "\tge_eeprom.py [serial_port_name] write < [input_file] \n")


def read_eeprom_pages(serial_port,
                      ilog_parser,
                      total_pages=32,
                      timeout=10):
    """
    ...
    for page_num in range(total_pages):
        try:
            ...
            while True:
                ...
                if page_data_read is not None:
                    sys.stdout.buffer.write(page_data_read)
                    break
                else:
            ...
            assert False, "EEPROM read page %d times out.\n" % page_num


def write_eeprom_pages(serial_port,
                       ilog_parser,
                       total_pages=32,
                       timeout=10):
    """
    ...
    for page_num in range(total_pages):
        try:
            ...
            while True:
                if attempt == 0:
                    page_data = sys.stdin.buffer.read(16)
            ...
            assert False, "EEPROM write page %d times out.\n" % page_num
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        ...
        if sys.argv[2] == 'read':
            try:
                read_eeprom_pages(sp, ilp)
            except Exception as e:
                traceback.print_exc(file=sys.stderr)
                sys.stderr.write("Failed to read data from EEPROM.\n")
        elif sys.argv[2] == 'write':
            try:
                write_eeprom_pages(sp, ilp)
            except Exception as e:
                traceback.print_exc(file=sys.stderr)
                sys.stderr.write("Failed to write data to EEPROM.\n")
        else:
            print_usage()
